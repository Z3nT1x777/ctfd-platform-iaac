...
- **OSINT Static Sync:** Synchronisation automatique des challenges OSINT statiques vers le serveur web via `scripts/sync_osint_static.py` (voir section dédiée)
...
## Synchronisation automatique des challenges OSINT statiques

Les challenges OSINT qui contiennent un dossier `resources/` sont automatiquement synchronisés vers le dossier web `/var/www/osint/` lors du déploiement (commande `scripts/sync_challenges_ctfd.py`).

- **Déclenchement** : la synchronisation s'exécute automatiquement à la fin du déploiement, aucune commande manuelle n'est requise.
- **Cible** : chaque dossier `challenges/osint/<challenge>/resources/` est copié dans `/var/www/osint/<challenge>/`.
- **Logs** :
    - Les dossiers copiés sont affichés en cyan (update ou create).
    - Les warnings (dossier manquant) sont affichés en jaune.
    - Un résumé indique le nombre de challenges synchronisés.
- **Dépannage** :
    - Si le script ou le dossier cible est absent, un message d'information s'affiche et la synchronisation est ignorée.
    - Pour forcer une resynchronisation, relancer `python scripts/sync_osint_static.py --target /var/www/osint/` dans la VM.

Voir aussi : [docs/README_CHALLENGES.md](docs/README_CHALLENGES.md#osint-static-sync)
